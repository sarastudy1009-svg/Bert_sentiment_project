...
from src_kim_jihyeo.config import LABEL_TO_ID, SEED


def load_sentiment_csv(data_path: str | Path) -> pd.DataFrame:
    """document, label 컬럼을 가진 CSV 파일을 읽어 정제된 DataFrame으로 반환합니다."""
    ...
    # 컬럼명 앞뒤 공백을 제거하여 " document " 같은 문제를 예방합니다.
    dataset.columns = dataset.columns.str.strip()

    # 학습에 필요한 document 컬럼과 label 컬럼이 있는지 확인합니다.
    required_columns = {"document", "label"}
    if not required_columns.issubset(set(dataset.columns)):
        raise ValueError("CSV 파일에는 document, label 컬럼이 반드시 있어야 합니다.")

    # 리뷰 또는 라벨이 비어 있는 행은 학습 오류를 막기 위해 제거합니다.
    dataset = dataset.dropna(subset=["document", "label"]).reset_index(drop=True)

    # label이 0.0/1.0 float으로 저장된 경우를 포함해 정수로 변환합니다.
    dataset["label"] = dataset["label"].map(lambda value: LABEL_TO_ID.get(value, LABEL_TO_ID.get(str(value).strip().lower())))

    # 변환되지 않은 라벨이 있으면 학습 전에 데이터 오류를 알려줍니다.
    if dataset["label"].isna().any():
        raise ValueError("label 컬럼은 0.0/1.0 또는 0/1 값이어야 합니다.")

    # Trainer가 요구하는 정수 라벨 형식으로 변환합니다.
    dataset["label"] = dataset["label"].astype(int)

    # document 컬럼을 문자열로 변환하여 숫자나 결측 혼합 데이터로 인한 토큰화 오류를 줄입니다.
    dataset["document"] = dataset["document"].astype(str)
