...
from src_kim_jihyeo.config import DEFAULT_DATA_PATH, DEFAULT_MODEL_NAME, MAX_LEN, MODEL_DIR, SEED
from src_kim_jihyeo.data_loader import load_sentiment_csv, split_dataset
from src_kim_jihyeo.dataset import BertSentimentDataset
from src_kim_jihyeo.modeling import apply_fine_tuning_strategy, count_trainable_parameters, create_model
from src_kim_jihyeo.utils import get_device, set_seed
...
def train(args: argparse.Namespace) -> None:
    ...
    train_dataset = BertSentimentDataset(train_df["document"], train_df["label"], tokenizer, max_len=args.max_len)

    # 검증 데이터도 같은 토크나이저와 최대 길이로 Dataset 객체를 만듭니다.
    valid_dataset = BertSentimentDataset(valid_df["document"], valid_df["label"], tokenizer, max_len=args.max_len)

    # 최종 평가를 위해 테스트 Dataset 객체도 준비합니다.
    test_dataset = BertSentimentDataset(test_df["document"], test_df["label"], tokenizer, max_len=args.max_len)
