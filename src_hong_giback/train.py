...
from src_hong_giback.config import DEFAULT_DATA_PATH, DEFAULT_MODEL_NAME, MAX_LEN, MODEL_DIR, SEED
from src_hong_giback.data_loader import load_sentiment_data, split_dataset
from src_hong_giback.dataset import BertSentimentDataset
from src_hong_giback.modeling import apply_fine_tuning_strategy, count_trainable_parameters, create_model
from src_hong_giback.utils import get_device, set_seed
...
def train(args: argparse.Namespace) -> None:
    ...
    dataset = load_sentiment_data(args.data_path)
    if args.sample_size and args.sample_size < len(dataset):
        dataset = dataset.sample(n=args.sample_size, random_state=SEED).reset_index(drop=True)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data_path", type=str, default=str(DEFAULT_DATA_PATH), help="ratings_train.txt 또는 감성분석 CSV 경로")
    ...
    parser.add_argument("--max_len", type=int, default=MAX_LEN, help="최대 입력 토큰 길이")
    ...
    parser.add_argument("--warmup_steps", type=int, default=0, help="warmup step 수")
    parser.add_argument("--weight_decay", type=float, default=0.01, help="weight decay 값")
    parser.add_argument("--sample_size", type=int, default=0, help="빠른 실험용 샘플 수. 0이면 전체 데이터 사용")
