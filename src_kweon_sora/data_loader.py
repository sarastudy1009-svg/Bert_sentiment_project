...
from src_kweon_sora.config import LABEL_TO_ID, SEED
...
def load_sentiment_csv(data_path: str | Path) -> pd.DataFrame:
    ...
    try:
        ...
        dataset = pd.read_table(data_path, encoding="cp949")

    # 컬럼명 앞뒤 공백을 제거하여 " review " 같은 문제를 예방합니다.
    dataset.columns = dataset.columns.str.strip()

    # ratings_train.txt 컬럼명을 기존 코드에 맞게 변경
    ...
    # 실습시간을 줄이기 위해 일부 데이터만 사용 (n개)
    dataset = dataset.sample(n=3000, random_state=SEED).reset_index(drop=True)
