...
from src_park_hyoejong.config import DEFAULT_DATA_PATH, DEFAULT_MODEL_NAME, MAX_LEN, MODEL_DIR, SEED
from src_park_hyoejong.data_loader import load_sentiment_csv, prepare_nsmc, split_dataset
from src_park_hyoejong.dataset import BertSentimentDataset
from src_park_hyoejong.modeling import apply_fine_tuning_strategy, count_trainable_parameters, create_model
from src_park_hyoejong.utils import get_device, set_seed
...
def train(args: argparse.Namespace) -> None:
    ...
    # NSMC 모드일 때 CSV가 없으면 자동으로 다운로드·변환합니다.
    if args.use_nsmc:
        data_path = prepare_nsmc(save_path=DEFAULT_DATA_PATH, force=args.force_download)
    else:
        data_path = args.data_path

    # CSV 파일을 읽고 라벨을 숫자로 변환합니다.
    dataset = load_sentiment_csv(args.data_path)
    print(f"[train] 전체 데이터: {len(dataset):,}건")

    # 데이터를 학습, 검증, 테스트 세트로 나눕니다.
    train_df, valid_df, test_df = split_dataset(dataset)
    print(f"[train] train {len(train_df):,} / valid {len(valid_df):,} / test {len(test_df):,}")
...
def parse_args() -> argparse.Namespace:
    ...
    # NSMC 자동 다운로드 여부 (기본값: True)
    parser.add_argument(
        "--use_nsmc", action="store_true", default=True,
        help="NSMC 데이터셋을 자동 다운로드·변환하여 사용합니다."
    )

    # NSMC 재다운로드 강제 여부
    parser.add_argument(
        "--force_download", action="store_true", default=False,
        help="이미 변환된 NSMC CSV가 있어도 재다운로드합니다."
    )

    # use_nsmc=False일 때 학습 CSV 파일 경로를 입력받습니다.
    parser.add_argument("--data_path", type=str, default=str(DEFAULT_DATA_PATH), help="use_nsmc=False일 때 review,sentiment 컬럼을 가진 CSV 파일 경로")
    ...
    # 평가 배치 크기를 입력받습니다.
    parser.add_argument("--eval_batch_size", type=int, default=64, help="평가 배치 크기")

    parser.add_argument("--learning_rate", type=float, default=3e-5)

    # warmup step 수를 입력받습니다.
    parser.add_argument("--warmup_steps", type=int, default=500, help="학습률 warmup step 수")
