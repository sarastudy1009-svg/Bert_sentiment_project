...
from src_park_hyoejong.config import DEFAULT_DATA_PATH, LABEL_TO_ID, SEED


import urllib.request

def prepare_nsmc(save_path: str | Path = DEFAULT_DATA_PATH, force: bool = False) -> Path:
    """NSMC TSV를 GitHub Raw에서 직접 다운로드하고 프로젝트 CSV 포맷으로 저장합니다."""
    save_path = Path(save_path)

    if save_path.exists() and not force:
        print(f"[prepare_nsmc] 기존 파일 사용: {save_path}")
        return save_path

    # NSMC 공식 GitHub Raw URL (e9t/nsmc)
    NSMC_URLS = {
    ...
    dfs = []
    for split, url in NSMC_URLS.items():
        print(f"[prepare_nsmc] {split} 다운로드 중: {url}")
        tmp_path = save_path.parent / f"nsmc_{split}_raw.txt"
        ...
        # 빈 리뷰 및 중복 제거
        df = df.dropna(subset=["document"]).drop_duplicates(subset=["document"])

        # 컬럼명 변환: document → review, label → sentiment
        df = df.rename(columns={"document": "review", "label": "sentiment"})

        # 숫자 라벨(0/1) → 한글 문자열(부정/긍정)
        df["sentiment"] = df["sentiment"].map({0: "부정", 1: "긍정"})

        dfs.append(df[["review", "sentiment"]])
    ...
    print(f"[prepare_nsmc] 저장 완료: {save_path}  (총 {len(full_df):,}건)")
