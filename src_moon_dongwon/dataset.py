...
from src_moon_dongwon.config import DEFAULT_MODEL_NAME, ID_TO_LABEL, MAX_LEN, MODEL_DIR
from src_moon_dongwon.utils import get_device
...
class SentimentPredictor:
    ...
    def predict(self, text: str) -> dict[str, float | str]:
        ...
        # ID_TO_LABEL 딕셔너리에서 예측된 라벨 문자열을 가져옵니다.
        # 만약 config에 지정되지 않았을 경우를 대비해 0: 부정을 디폴트 포맷으로 방어 코드를 작성합니다.
        label_string = ID_TO_LABEL.get(predicted_id, "부정" if predicted_id == 0 else "긍정")
