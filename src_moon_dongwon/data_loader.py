...
TARGET_DATA_PATH = Path(__file__).resolve().parent.parent / "data" / "review_statement_enhanced.csv"


def load_sentiment_csv(data_path: str | Path = TARGET_DATA_PATH) -> pd.DataFrame:
    """review_statement_enhanced.csv 파일을 읽어 BERT 학습에 맞는 정제된 DataFrame으로 반환합니다."""
    ...
    if not required_columns.issubset(set(dataset.columns)):
        raise ValueError(f"CSV 파일({data_path.name})에는 review, sentiment 컬럼이 반드시 있어야 합니다.")
