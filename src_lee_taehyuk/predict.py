"""저장된 한국어 BERT 감성분석 모델을 불러와 문장 긍정/부정을 예측하는 파일입니다."""
...
from src_lee_taehyuk.config import DEFAULT_MODEL_NAME, ID_TO_LABEL, MAX_LEN, MODEL_DIR
from src_lee_taehyuk.utils import get_device
...
class SentimentPredictor:
    ...
    def __init__(self, model_dir: str | Path = MODEL_DIR, fallback_model_name: str = DEFAULT_MODEL_NAME, max_len: int = MAX_LEN):
        ...
        # 학습 때 사용한 것과 같은 Tokenizer를 불러옵니다. (klue/bert-base 등 한국어 모델 호환)
        self.tokenizer = AutoTokenizer.from_pretrained(self.load_path)

        # 이진 분류용 한국어 BERT 모델을 불러옵니다.
