"""한국어 BERT 분류 모델 생성과 Fine-tuning 범위 설정을 담당하는 파일입니다."""

from transformers import AutoModelForSequenceClassification, PreTrainedModel


def create_model(model_name: str, num_labels: int = 2) -> PreTrainedModel:
    """Hugging Face Hub 또는 로컬 경로에서 한국어 BERT 문장 분류 모델을 불러옵니다."""
    # AutoModelForSequenceClassification은 klue/bert-base 등 한국어 BERT 계열도 자동으로 인식합니다.
...
def apply_fine_tuning_strategy(model: PreTrainedModel, strategy: int = 3) -> PreTrainedModel:
    ...
    # BERT Base 모델의 마지막 Encoder Layer 이름입니다. (klue/bert-base도 동일한 구조를 따릅니다)
    last_encoder_layer_name = "encoder.layer.11"

    # AutoModelForSequenceClassification으로 불러온 모델의 BERT 본체에 접근합니다.
    # klue/bert-base 등 표준 BERT 구조는 'bert' 속성으로 본체에 접근할 수 있습니다.
    bert_backbone = model.bert

    if strategy == 1:
        # 전략 1은 BERT 본체 전체를 동결하고 classifier만 학습합니다.
        for name, param in bert_backbone.named_parameters():
